...
自包含的命令行应用开发指南：命令结构、参数选项、子命令组、输出格式化、交互确认、配置管理、错误处理、测试与打包入口。新项目推荐 Typer（类型注解驱动、API 简洁）；复杂命令树/自定义补全用 Click。所有示例遵循 `python-standards` 技能（类型注解、中文 docstring、`from __future__ import annotations`）。
...
决策准则：
- **新项目首选 Typer**：类型注解驱动、代码量少、自动生成帮助，与 `python-standards` 技能 类型注解要求天然契合。
...
- 帮助文本用中文，与 `python-standards` 技能 一致。
...
- 禁止 `except Exception: pass`（见 `python-standards` 技能）；捕获后必须记录/包装/重抛。
...
- 环境变量用 `monkeypatch.setenv`（见 `python-standards` 技能 Mock 优先级）。
- 交互式命令测试：`input="值\n值\n"` 模拟 stdin 逐行输入。
...
- `subprocess.run` 必须加 `check=True`（ruff `PLW1510`）；禁用 `shell=True`（见 `python-standards` 技能 安全章节）。
