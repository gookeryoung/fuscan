...
PySide 开发硬约束简表，覆盖架构与分层、配置与资源、控件与窗口生命周期、性能。详细设计令牌、四区布局规范、UI 设计规范、实现模式与代码模板见本技能 后续章节及 `UI-DESIGN.md`/`LAYOUT.md`/`PATTERNS.md`。
...
```python
"""fuscan GUI 入口（PySide2/PySide6 双兼容）。"""
...
from fuscan import theme
from fuscan.widgets.sidebar import Sidebar
...
class MainWindow(QMainWindow):
    ...
    def _build_header(self) -> QFrame:
        ...
        title = QLabel("fuscan", objectName="headerTitle")
