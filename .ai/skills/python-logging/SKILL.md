...
自包含的日志指南：标准库 `logging` 全链路实践，涵盖模块级 Logger 约定、`dictConfig` 配置、文件轮转、自定义 Formatter、结构化 JSON 输出、分层策略、GUI/CLI 集成、性能与脱敏。禁止 `print` 残留（`python-standards` 技能 强制），日志用 `%` 延迟格式化（不用 f-string），凭证/密码不进日志。所有示例遵循 `python-standards` 技能（`from __future__ import annotations`、中文 docstring、类型注解）。
...
PySide GUI 中将日志输出到 `QTextEdit` 面板；跨线程日志须通过信号槽转发到主线程（参考 `gui-pyside` 技能 的 QThread/Signal 模式）。
...
- `MAX_LINES` 截断头部防止内存膨胀；级别颜色用令牌（见 `gui-pyside` 技能 的 `theme.py`）。
...
要点：
- **延迟格式化**：`logger.debug("x=%s", x)` 而非 `logger.debug(f"x={x}")`；`python-standards` 技能 强制 `%` 风格。
...
1. **`print` 残留**：开发期 `print` 调试未删除，污染 stdout。`python-standards` 技能 禁止；一律用 `logger.debug`。
