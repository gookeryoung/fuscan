...
自包含的性能工程指南：测量先行 → 建立基线 → 定位热点 → 针对性优化 → 回归门禁。遵循 `python-standards` 技能 硬约束：`from __future__ import annotations`、中文 docstring、`subprocess` 禁 `shell=True`。核心原则：**未测量不优化**——凭直觉的"优化"多数是噪声，必须用数据驱动。
...
`functools.lru_cache` 缓存纯函数结果；`functools.cached_property` 缓存实例属性（`python-standards` 技能 数据结构章节）。
...
I/O 密集场景用 `asyncio` 并发多个 I/O 操作（详见 `python-concurrency` 技能）。
