...
> 适用于所有 AI 编程助手。编写或审查 Python 代码、需要跨领域通用硬约束参考时查阅。
> 各领域详细模式与代码模板见对应专项技能文档。
...
详细参考：工具链配置拆分决策与完整示例见 `python-project-structure` 技能；覆盖率排除规则与 `# pragma: no cover` 见 `python-testing` 技能。
...
- 任务完成后自动 `git add`（按文件名）+ `git commit`（遵循 Git 提交规则）+ `git push`（分支已跟踪远程时；新分支跳过并在总结说明）。
- 破坏性命令的暂停边界遵循开发流程规则中的暂停条件：`force-push`、工作区有改动时的 `reset --hard`、`git clean -f`/`-fd`/`-fx` 需暂停；`git clean -fX`、`git stash`/`stash pop`、`git revert`、工作区干净时的 `reset --hard` 可自主执行。
- staging 按文件名添加，不用 `git add -A`/`git add .`。

## 详细参考

本技能为硬约束简表，各领域详细模式与代码模板见对应技能文档：
