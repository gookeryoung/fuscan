...
自包含的外部命令执行指南：`subprocess.run`、`Popen`、命令构造、输出捕获、流式读取、超时管理、管道链式。遵循 `python-standards` 技能 安全要求：禁用 `shell=True`（优先 `list[str]` 形式），`subprocess.run` 必须 `check=True`（ruff `PLW1510`），编码显式 `encoding="utf-8"`。
...
命令必须以 `list[str]` 形式传递，每个参数独立元素；禁止 `shell=True`（`python-standards` 技能 安全要求）。
...
`python-standards` 技能 强制：禁用 `shell=True`，参数来自用户输入时必须用 `list[str]` 形式或 `shlex.quote`。
...
要点：
- **禁用 `shell=True`**（`python-standards` 技能 硬约束）：`list[str]` 形式直接传 `execv`，不经 shell，天然防注入。
...
在 PySide 中运行子进程须用 QThread Worker 模式（参考 `gui-pyside` 技能）：Worker 在后台线程 `run` 子进程，通过信号将输出转发到主线程更新 UI。
...
1. **`shell=True` 命令注入**：`subprocess.run(f"ls {user}", shell=True)` 中 `user="; rm -rf /"` 会执行 rm。禁用 `shell=True`，用 `list[str]` 形式（`python-standards` 技能 硬约束）。
2. **缺 `check=True`**：非零退出码被静默忽略，错误隐藏。ruff `PLW1510` 强制 `check=True`（`python-standards` 技能 工具链）。
