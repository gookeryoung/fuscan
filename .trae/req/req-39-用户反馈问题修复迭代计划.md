...
- [ ] 问题1：同一任务下，文件被删除后扫描结果未更新（增量扫描未清理已删除文件的结果）
- [ ] 问题3：压缩文件扫描极慢，扫描任务始终无法结束（死循环或无超时）
- [ ] 问题10：扫描过程中切换到设置页出现明显卡顿
...
- [ ] 问题2：展开区「切换目标」右侧增加「重新扫描」按钮（全量扫描，非增量）
- [ ] 问题4：PDF 报告生成接通 GUI（WorkspaceCard 导出菜单 + ResultsPage 导出按钮）
- [ ] 问题5：设置 Tab 顺序调整，扫描页放在通用前面
- [ ] 问题6：定义规则页面取消模板功能（移除模板按钮与对话框）
- [ ] 问题9：XML 等通用文件类型标注在文件类型标签中
...
- [ ] 问题7：规则配置移到首页下方（添加任务上方），作为全局配置而非工作任务配置
- [ ] 问题8：工作任务把展开区的「切换目标」挪到当前「定义规则」的位置
