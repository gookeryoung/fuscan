...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    各启动阶段通过 :class:`~fuscan.perf.timed` 分段计时并打印用时，外层
    ``timed("启动流程")`` 汇总总用时。仅在性能测量启用时（``FUSCAN_PERF=1``
    环境变量或 CLI ``--perf``）输出，发布版默认关闭、零开销。
    """
    ...
    # 外层计时汇总总启动用时；退出（含提前 return）时 __exit__ 自动记录
    with timed("启动流程"):
        with timed("构造 QGuiApplication"):
        ...
        with timed("注册 QML 类型"):
            register_qml_types()

        with timed("构造主控制器"):
            controller = AppController()

        with timed("构造 QML 引擎并注册上下文"):
