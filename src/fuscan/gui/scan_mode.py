...
（``"full"``/``"drive"``/``"folder"``），QML 侧用索引切换模式更直观，
...
# 索引 → 模式字符串（顺序与 QML 切换控件一致：0=全盘 / 1=盘符 / 2=文件夹）
SCAN_MODE_INDEX_TO_STR: tuple[str, ...] = ("full", "drive", "folder")
...
SCAN_MODE_DEFAULT_INDEX: int = SCAN_MODE_STR_TO_INDEX.get("folder", 2)


def scan_mode_text(mode_str: str) -> str:
    """按模式字符串取中文展示文本，未知模式回退为原字符串。

    :param mode_str: 模式字符串（``"full"``/``"drive"``/``"folder"``）
    :return: 中文展示文本
    """
