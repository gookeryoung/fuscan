...
class TestScanModeIndexToStr:
    def test_valid_indices(self) -> None:
        """0/1/2 索引应映射到对应模式字符串。"""
        assert scan_mode_index_to_str(0) == "full"
        assert scan_mode_index_to_str(1) == "drive"
        assert scan_mode_index_to_str(2) == "folder"
...
class TestScanModeStrToIndex:
    def test_known_modes(self) -> None:
        """已知模式字符串返回对应索引。"""
        assert scan_mode_str_to_index("full") == 0
        assert scan_mode_str_to_index("drive") == 1
        assert scan_mode_str_to_index("folder") == 2
...
class TestScanModeText:
    def test_known_modes_text(self) -> None:
        """已知模式返回中文展示文本。"""
        assert scan_mode_text("full") == "全盘扫描"
...
class TestConstantsConsistency:
    ...
    def test_default_index_is_folder(self) -> None:
        ...
        assert SCAN_MODE_DEFAULT_INDEX == 2
