...
class TestScannerEntropyIntegration:
    """Scanner 高熵字符串兜底检测集成测试（``entropy_enabled=True``）。

    覆盖 ``Scanner._detect_entropy`` 及其在主扫描路径的调用点：熵检测命中、
    空内容跳过、内容读取异常、``find_high_entropy_strings`` 异常、超长 token 截断。
    这些分支此前无端到端测试触达（entropy.py 自身已 100%，但 Scanner 集成路径未覆盖），
    与运行平台无关，属真实覆盖缺口。
    """

    # base64 编码的 48 字节随机数据：高熵，确保触发 find_high_entropy_strings 命中
    _HIGH_ENTROPY_TOKEN = "aB3xK9mQ7pL2wR5vT8nY4cF6hJ0dG1sZ" + "eU3iO7kP9lM2nB5vC8xZ4qW6rT0yA1"

    def test_entropy_hit_reported(self, tmp_path: Path) -> None:
        """启用熵检测且文件含高熵串时，应产出 ENTROPY 命中。"""
        from fuscan.scanner.entropy import ENTROPY_RULE_NAME

        (tmp_path / "secret.txt").write_text(f"token = {self._HIGH_ENTROPY_TOKEN}\n", encoding="utf-8")
        # 无普通规则，仅靠熵检测兜底
        rs = _build_ruleset(_filename_rule("noop", "不会匹配的名字"))
        scanner = Scanner(rs, entropy_enabled=True, entropy_threshold=3.5)
        report = scanner.scan(tmp_path)
        entropy_hits = [h for r in report.hits for h in r.hits if h.rule_name == ENTROPY_RULE_NAME]
        assert entropy_hits, "应至少有一条高熵命中"
        assert "高熵字符串" in entropy_hits[0].detail

    def test_entropy_disabled_no_hit(self, tmp_path: Path) -> None:
        """未启用熵检测时，高熵串不产生 ENTROPY 命中。"""
        from fuscan.scanner.entropy import ENTROPY_RULE_NAME

        (tmp_path / "secret.txt").write_text(f"token = {self._HIGH_ENTROPY_TOKEN}\n", encoding="utf-8")
        rs = _build_ruleset(_filename_rule("noop", "不会匹配的名字"))
        scanner = Scanner(rs, entropy_enabled=False)
        report = scanner.scan(tmp_path)
        entropy_hits = [h for r in report.hits for h in r.hits if h.rule_name == ENTROPY_RULE_NAME]
        assert not entropy_hits

    def test_detect_entropy_empty_content_returns_empty(self, tmp_path: Path) -> None:
        """内容为空时 _detect_entropy 直接返回空列表（覆盖 not content 分支）。"""
        from fuscan.scanner.context import FileEntry, MatchContext

        rs = _build_ruleset(_filename_rule("noop", "x"))
        scanner = Scanner(rs, entropy_enabled=True)
        entry = FileEntry(path=tmp_path / "e.txt", name="e.txt", size=0, mtime=0.0, extension="txt")
        ctx = MatchContext(entry, content_provider=lambda _e: "")
        assert scanner._detect_entropy(entry, ctx) == []

    def test_detect_entropy_content_read_error_returns_empty(self, tmp_path: Path) -> None:
        """content 读取抛异常时 _detect_entropy 捕获并返回空列表（覆盖 except 分支）。"""
        from fuscan.scanner.context import FileEntry, MatchContext

        rs = _build_ruleset(_filename_rule("noop", "x"))
        scanner = Scanner(rs, entropy_enabled=True)
        entry = FileEntry(path=tmp_path / "e.txt", name="e.txt", size=1, mtime=0.0, extension="txt")

        def boom(_e: FileEntry) -> str:
            raise OSError("读取失败")

        ctx = MatchContext(entry, content_provider=boom)
        assert scanner._detect_entropy(entry, ctx) == []

    def test_detect_entropy_find_error_returns_empty(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """find_high_entropy_strings 抛异常时 _detect_entropy 捕获并返回空列表。"""
        import fuscan.scanner.scanner as scanner_mod
        from fuscan.scanner.context import FileEntry, MatchContext

        rs = _build_ruleset(_filename_rule("noop", "x"))
        scanner = Scanner(rs, entropy_enabled=True)
        entry = FileEntry(path=tmp_path / "e.txt", name="e.txt", size=10, mtime=0.0, extension="txt")

        def boom(content: str, *, threshold: float) -> list[tuple[str, float]]:
            raise RuntimeError("熵计算失败")

        monkeypatch.setattr(scanner_mod, "find_high_entropy_strings", boom)
        ctx = MatchContext(entry, content_provider=lambda _e: "some content here")
        assert scanner._detect_entropy(entry, ctx) == []

    def test_detect_entropy_truncates_long_token(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """超长 token（>80 字符）的 detail 应截断为 前32 + ... + 后32。"""
        import fuscan.scanner.scanner as scanner_mod
        from fuscan.scanner.context import FileEntry, MatchContext

        rs = _build_ruleset(_filename_rule("noop", "x"))
        scanner = Scanner(rs, entropy_enabled=True)
        entry = FileEntry(path=tmp_path / "e.txt", name="e.txt", size=200, mtime=0.0, extension="txt")
        long_token = "A" * 100

        def fake_find(content: str, *, threshold: float) -> list[tuple[str, float]]:
            return [(long_token, 5.0)]

        monkeypatch.setattr(scanner_mod, "find_high_entropy_strings", fake_find)
        ctx = MatchContext(entry, content_provider=lambda _e: long_token)
        hits = scanner._detect_entropy(entry, ctx)
        assert len(hits) == 1
        # match_text 保留完整 token，detail 中展示截断形式
        assert hits[0].match_text == long_token
        assert "..." in hits[0].detail
        assert f"{long_token[:32]}...{long_token[-32:]}" in hits[0].detail
